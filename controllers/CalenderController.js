...
class CalendarController {
  ...
  static async displayAllTimeSlots(req, res) {
    ...
    function convertToNewYorkTime(slots) {
      return slots.map((slot) => {
        const newYorkStartTime = moment(slot.startTime)
          .tz(userTimeZone)
          .format();
        const newYorkEndTime = moment(slot.endTime)
          .tz(userTimeZone)
          .format();
    ...
    function getDatesWithAppointments(updatedSlots) {
      const dates = {}; // Use an object to store dates

      updatedSlots.forEach((slot) => {
        const date = moment(slot.startTime).format('YYYY-MM-DD'); // Extract the date part
        dates[date] = true; // Set the date as a key in the object with the value true
      });
    ...
    try {
      ...
      const updatedSlots = convertToNewYorkTime(slots);

      return res
        .status(200)
        .json(getDatesWithAppointments(updatedSlots));
    } catch (error) {
  ...
  static async getEventsByDate(req, res) {
    const requestDate = new Date(req.body.navigationDate);
    try {
      const slotsByDate = await CalendarDAO.getSlotsByDate(
        requestDate
      );
